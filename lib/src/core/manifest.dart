...
class ManifestFileRecord {
  ...
  factory ManifestFileRecord.fromJson(Map<String, Object?> json) => ManifestFileRecord(
...
class ManifestReadError {
  ...
  factory ManifestReadError.fromJson(Map<String, Object?> json) => ManifestReadError(
        path: json['path']! as String,
...
class CentraManifest {
  ...
  }) : files = (files.toList()..sort((left, right) => left.path.compareTo(right.path)));
  ...
  Map<String, Object?> toJson() => <String, Object?>{
        ...
        'source': source,
        'algorithms': algorithms.map((algorithm) => algorithm.toJson()).toList(growable: false),
  ...
  factory CentraManifest.fromJson(Map<String, Object?> json) {
    ...
    return CentraManifest(
      ...
      algorithms: (json['algorithms']! as List<Object?>)
          .map((value) => HashAlgorithmDescriptor.fromJson((value! as Map).cast<String, Object?>()))
      ...
      files: (json['files']! as List<Object?>)
          .map((value) => ManifestFileRecord.fromJson((value! as Map).cast<String, Object?>()))
          .toList(growable: false),
      errors: (json['errors'] as List<Object?>? ?? const <Object?>[])
          .map((value) => ManifestReadError.fromJson((value! as Map).cast<String, Object?>()))
          .toList(growable: false),
...
class ManifestDiff {
  ...
  int count(ManifestChangeType type) => changes.where((change) => change.type == type).length;

  bool get hasIntegrityChanges => changes.any(
        (change) => change.type == ManifestChangeType.added ||
...
class ManifestComparator {
  ...
  ManifestDiff compare(CentraManifest before, CentraManifest after) {
    final beforeByPath = <String, ManifestFileRecord>{for (final file in before.files) file.path: file};
    final afterByPath = <String, ManifestFileRecord>{for (final file in after.files) file.path: file};
    final paths = <String>{...beforeByPath.keys, ...afterByPath.keys}.toList()..sort();
    ...
    for (final path in paths) {
      ...
      final algorithmIds = <String>{...oldFile.digests.keys, ...newFile.digests.keys}.toList()..sort();
      final changedAlgorithms = <String>[];
      for (final id in algorithmIds) {
        final left = oldFile.digests[id];
        final right = newFile.digests[id];
        if (left == null || right == null || !constantTimeHexEquals(left, right)) {
          changedAlgorithms.add(id);
        }
      }
      if (changedAlgorithms.isNotEmpty || oldFile.size != newFile.size || oldFile.symlinkTarget != newFile.symlinkTarget) {
        changes.add(ManifestChange(
          path: path,
      ...
      final metadataChanged = oldFile.modifiedAt?.toUtc() != newFile.modifiedAt?.toUtc() || oldFile.mode != newFile.mode;
      changes.add(ManifestChange(
        path: path,
        type: metadataChanged ? ManifestChangeType.metadata : ManifestChangeType.unchanged,
