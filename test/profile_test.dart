...
void main() {
  group('CentraProfile validation', () {
    ...
    test('requires an explicit output format', () {
      ...
      expect(
        profile.validate(),
        contains('Select at least one output format.'),
      );
    });
    ...
    test('allows MD5 compatibility profile without hiding its status', () {
      final profile = testProfile(
        root: '/tmp/project',
        algorithmIds: const <String>['md5'],
      );
      expect(profile.validate(), isEmpty);
      expect(
        AlgorithmRegistry().descriptor(profile.algorithmIds.single).status,
    ...
    test('validates source-specific required fields', () {
      ...
      expect(
        ssh.validate(),
        containsAll(<String>['SSH host is required.', 'SSH user is required.']),
      );

      const container = SourceConfig(
