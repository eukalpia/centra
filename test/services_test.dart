...
void main() {
  ...
  group('OutputService', () {
    ...
    test(
      'writes canonical, compatibility, report and encrypted ZIP outputs',
      () async {
        ...
        final result = await OutputService().write(
          profile,
          manifest,
        ...
        final shaText = await File(
          '${outputDirectory.path}/hash_values.sha256.txt',
        ).readAsString();
        final md5Text = await File(
          '${outputDirectory.path}/hash_values.md5.txt',
        ).readAsString();
  ...
  group('SignatureService', () {
    ...
    test(
      'generates, signs and verifies an Ed25519 manifest signature',
      () async {
    ...
    test('private and public key documents round-trip', () async {
      ...
      try {
        ...
        final decodedPrivate = service.decodePrivateKey(
          await privateFile.readAsString(),
        );
        final decodedPublic = service.decodePublicKey(
    ...
    test(
      'rejects malformed key IDs and unsupported signature schemas',
      () async {
        final service = SignatureService(clock: () => fixed);
