...
void main() {
  group('IntegrityScanner', () {
    ...
    test('hashes multiple algorithms while applying exclusions', () async {
      ...
      expect(result.manifest.files.map((file) => file.path), <String>[
        'README.md',
        'lib/main.dart',
      ]);
    ...
    test(
      'produces the same content records regardless of worker count',
      () async {
        project = await createProject(<String, String>{
          for (var index = 0; index < 25; index++)
            'files/$index.txt': 'value-$index',
        });
        final oneWorker = await IntegrityScanner(
          random: Random(2),
        ).scan(testProfile(root: project.path, workerCount: 1));
        final eightWorkers = await IntegrityScanner(
          random: Random(3),
        ).scan(testProfile(root: project.path, workerCount: 8));
        expect(
    ...
    test('runs an external custom hash command without a shell', () async {
      ...
      expect(
        result.manifest.files.single.digests['external-test'],
        '0123456789abcdef',
      );
    ...
    test(
      'records symlink targets instead of following them when requested',
      () async {
        ...
        final linkRecord = result.manifest.files.singleWhere(
          (file) => file.path == 'link.txt',
        );
